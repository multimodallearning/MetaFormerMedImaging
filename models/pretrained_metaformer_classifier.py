...
class PretrainedMetaformer(ClassifierBase):
    def __init__(self, ds_name, tokenmixer: str, patch_size: int = 224, kernel_size: int = 5,
        ...
        self.model = getattr(mf, model_name)(pretrained=True)
        # freeze pretrained weights if requested
        if freeze_pretrained:
            for name, param in self.model.named_parameters():
                if name.startswith('patch_embed.') or (
                        name.startswith('network.') and int(name.split('.')[1]) <= 2):
    ...
    def on_fit_start(self) -> None:
        if Task.current_task() is not None:
            state_pretrained = 'frozen' if self.hparams.freeze_pretrained else 'finetuned'
            Task.current_task().set_name(
                f'metaformer_{self.hparams.tokenmixer}_{state_pretrained}_{self.hparams.ds_name}')
