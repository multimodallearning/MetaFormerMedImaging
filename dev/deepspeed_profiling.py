...
for kernel_size in tqdm([3, 5, 7, 9], desc='Kernel'):
    for config in [(64, 128), (128, 64), (256, 32), (512, 16), (1024, 8), (2048, 4)]:
        ...
        flops, macs, params = profile(pool, (1, n_channel, spatial_dim, spatial_dim))
        results_df.loc[len(results_df.index)] = ['pool', kernel_label, spatial_label, flops, macs, params]

        flex = FlexTokenBlock(4, [spatial_dim, spatial_dim], 'cuda', n_channel, kernel_size).cuda()
        try:
            flops, macs, params = profile(flex, (1, n_channel, spatial_dim, spatial_dim))
            results_df.loc[len(results_df.index)] = ['flex', kernel_label, spatial_label, flops, macs, params]
        except:
            results_df.loc[len(results_df.index)] = ['flex', kernel_label, spatial_label, 'N/A', 'N/A', 'N/A']
