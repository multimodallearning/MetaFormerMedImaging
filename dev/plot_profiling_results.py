...
for spatial_dim, g_df in df.groupby('spatial'):
    ...
    sns.lineplot(x='kernel', y='flops', hue='operation', data=g_df, ax=axs[0])
    axs[0].hlines(row_full_atn['flops'].values, **full_attn_hline_kwargs)
    axs[0].legend()
    axs[0].set_title('FLOPS')

    sns.lineplot(x='kernel', y='macs', hue='operation', data=g_df, ax=axs[1])
    axs[1].hlines(row_full_atn['macs'].values, **full_attn_hline_kwargs)
    axs[1].legend()
    axs[1].set_title('MACs')

    sns.lineplot(x='kernel', y='params', hue='operation', data=g_df, ax=axs[2])
